...
3. `git remote`：查看／註冊遠端數據庫 remote repository 的設定
   ...
   │ `origin`、`upstream` 皆為常見的遠端數據庫名稱別名，亦可更改為其它遠端數據庫名稱
   │ 即 `origin/master` 就是 Remote Repository 的 master 分支
...
1. `git config`：建構 Git 相關設定
   ├ `git config --global http.proxy http://[FQDN]:[PORT]`：proxy 設定
...
2. `git commit`：將檔案從 Staging Area（暫存區） commit 到 Local Repository（本地儲存庫）
   │ 需要使用 `-m` 參數記述備註，並會分配一個 commit ID
...
3. `git checkout [branch_name]`：切換至指定的分支
   ...
   ├ `git checkout [branch_name] .`：將當前分支的所有檔案複製至指定分支
   └ ※在最後方有一個 `.`
...
3. `git merge [branch_name]`：將 `指定分支` 的檔案修改合併至 `當前分支`
   ...
   ├ 預設時使用 `fast-forward` 方式合併，不會產生新的 commit
   ...
   │ 將從 `git fetch` 取得的 Remote Repository 的變更
   └ 反應至 Local Repository 裡目前的 branch
...
- 和 `fork` 同樣是 Github／Bitbucket 上的整合功能
  ├ 不同於 `clone` 是將 `遠端repo` 複製至本地端
  ├ `fork` 指的是將 `他人的遠端repo` 複製至 `自己的遠端repo`
  └ 以便做出個人版本的內容修改
